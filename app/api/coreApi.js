...
function tryCacheThenRpcApi(cache, cacheKey, cacheMaxAge, rpcApiFunction, cacheConditionFunction) {
	if (config.btcExpIsDilithium && !rpcApiFunction.name.includes("Dilithium")) {
		// Attempt to map to Dilithium API if method exists
		var funcName = rpcApiFunction.name;
		if (funcName && dilithiumApi[funcName]) {
			rpcApiFunction = dilithiumApi[funcName];
		} else {
			// debugLog("No Dilithium mapping for: " + funcName);
		}
	}
